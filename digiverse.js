...
class Digiverse {
  ...
  async login(data, proxy, index, user) {
  ...
  async getBalance(token, proxy, index, user) {
  ...
  async checkin(token, proxy, index, user) {
  ...
  async getTaskList(token, proxy, index, user) {
  ...
  async completeTask(token, proxy, index, user, task) {
  ...
  async claimTask(token, proxy, index, user, task) {
  ...
  async farming(token, proxy, index, user) {
...
async rewardFarming(token, proxy, index, user) {
...
async claimFarming(token, proxy, index, user) { 
...
async gameCount(token, proxy, index, user) {
...
async claimGame(token, proxy, index, game_id) {
  ...
  async process(data, proxy, index) {
    ...
    const token = await this.login(data, proxy, index, user);
    if (token) {
      await this.getBalance(token, proxy, index, user);
      await this.checkin(token, proxy, index, user);
      if (config.is_do_task) {
        const taskList = await this.getTaskList(token, proxy, index, user);
        if (taskList) {
          for (const task of taskList["Follow Socials"]) {
            if (!task?.complete) {
              await this.completeTask(token, proxy, index, user, task);
              await this.sleep(5000);
              await this.claimTask(token, proxy, index, user, task);
            }
          }
      ...
      let next_claim = 0;
      const time = await this.rewardFarming(token, proxy, index, user);
      if (time) {
        next_claim = time;
      }
      if (next_claim === 0) {
        await this.farming(token, proxy, index, user);
      }
        ...
        wait_time = Math.min(wait_time,parseInt((next_claim - Date.now()) / 1000));
            ...
            await this.claimFarming(token, proxy, index, user);
            await this.farming(token, proxy, index, user);
        }
        let gameInfo = {
            game_count: 1
        ...
        while (gameInfo?.game_count > 0) {
            gameInfo = await this.gameCount(token, proxy, index, user);
            if (gameInfo?.game_count > 0) {
                this.log(`[Account ${index}] Bắt đầu chơi game, chờ 30s để hoàn thành...`.blue);
                await this.sleep(30000);
                await this.claimGame(token, proxy, index, gameInfo?.game_id);
            }
            else {
                break;
            }
